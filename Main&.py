...
class images:
    ...
    iCoin = [pygame.image.load('C:/Users/ayash/PycharmProjects/TowerBattles/Test Bullet.png')]
...
class Player:
    def __init__(self, WalkR, WalkL, Sprites, x, y):
        print(pygame.sprite.Sprite(playerG), "ioahd")
    ...
    def block(self):
        ...
        for power in powerL:
            if power.x == wall_r * round(self.x / wall_r) and power.y == wall_r * round((self.y + 37) / wall_r):
                return
...
class Power:
        ...
        try:
            ...
            for wall in wL:
                if wall.x == self.x and wall.y == self.y:
                    return
            #for power in powerL:
            #    if power.x == self.x and power.y == self.y:
            #        powerL = powerL[:-1]
            #        return
        except IndexError:
            return
    ...
    def update(self):
        if not self.glitch:
            ...
            try:
                mainSurf.blit(self.sprites[1][round(self.spriteC/4)], (self.x, self.y))
                self.spriteC += 1
